...
public class RyanairInterconnectedFlightsService
  ...
  public Flux<InterconnectedFlightDto> getFlights(
    ...
    boolean departureAndArrivalIsSameDay =
      departureDateTime.getDayOfYear() == arrivalDateTime.getDayOfYear() &&
      arrivalDateTime.compareTo(arrivalDateTime) == 0;

    if (!departureAndArrivalIsSameDay) throw new ResponseStatusException(
  ...
  private Mono<List<InterconnectedFlightDto>> getAllInterconnectedFlightsForRoute(
    ...
    Mono<List<LegDto>> allFlightsInRouteMono =
      this.getAllPossibleFlightsInRoute(route, localDate).collectList();
    ...
    Mono<List<InterconnectedFlightDto>> interconnectedFlightsMono = allFlightsInRouteMono.map(allFlightsInRoute -> {
        ...
        for (LegDto flight : allFlightsInRoute) {
          ...
          if (
            isDepartureAirport &&
            flight.getDepartureDateTime().isAfter(minDepartureTime)
          ) {
            sourceVertices.add(flight);
          }
          boolean isArrivalAirport =
            flight.getArrivalAirport() == destinationAirport;
          if (
            isArrivalAirport &&
            flight.getArrivalDateTime().isBefore(maxArrivalTime)
          ) {
          ...
          for (LegDto vertex : allVertices) {
            boolean vertexIsPreviousLeg =
              flight.getDepartureAirport() == vertex.getArrivalAirport();
  ...
  private boolean isValidFlight(
    ...
    boolean previousFlightValid = previousLegArrivalTime.isAfter(
      previousLegDepartureTime
    );
    boolean flightValid = arrivalTime.isAfter(departureTime);

    boolean departureTimeValid =
      departureTime.isAfter(
        previousLegArrivalTime.plusMinutes(minTransferMinutes)
      ) &&
      departureTime.isAfter(minDepartureTime);
